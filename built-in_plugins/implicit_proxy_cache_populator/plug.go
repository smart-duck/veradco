...
import (
	...
	admissioncontroller "github.com/smart-duck/veradco/veradco"
...
func (qp QueuedPull) clone() QueuedPull {
	return QueuedPull{url: qp.url, configuration: qp.configuration, dryRun: qp.dryRun, nbOfTries: qp.nbOfTries}
}
...
func (plug *ImplicitProxyCachePopulator) replicateImage(queuedPull QueuedPull) {

	url := queuedPull.url
	configuration := *queuedPull.configuration
	dryRun := queuedPull.dryRun
	nbOfTries := queuedPull.nbOfTries
	...
	if _, ok := plug.proceededImages[url]; !ok {
		...
		if hardErr != nil {
			...
			if nbOfTries < nbOfTriesMax {
				clonedQueuedPull := queuedPull.clone()
				clonedQueuedPull.nbOfTries++
				log.Infof("[IPCP] Error replicating image %s (retry #%d): %v\n", url, queuedPull.nbOfTries, hardErr)
				go func() { plug.pullQueue <- clonedQueuedPull }()
			} else {
				log.Infof("[IPCP] Error replicating image %s (stop retrying): %v\n", url, hardErr)
			}
		}
	} else {
